// array for the users search history
...
let getCityWeather = function(city) {
    // OpenWeather api url
    let apiUrl = "https://api.openweathermap.org/data/2.5/weather?q=" + city + "&appid=f6ff370ef5d393bd11cef4b7507dd5b4" + "&units=imperial";
    ...
    fetch(apiUrl)
        ...
        // error message if OpenWeather response fails
        .catch(function(error) {
            alert("OpenWeather Connection Failed");
        })
};
...
let searchSubmitHandler = function(event) {
    ...
    // to check if the search field has a value
    if(cityName) {
...
// function to display the details from openweathermap.org
let displayWeather = function(weatherData) {

    // format and display the values
    $("#main-city-name").text(weatherData.name + " (" + dayjs(weatherData.dt * 1000).format("MM/DD/YY") + ") ").append(`<img src="https://openweathermap.org/img/wn/${weatherData.weather[0].icon}@2x.png"></img>`);
    $("#main-city-temp").text("Temperature: " + weatherData.main.temp.toFixed(1) + "°F");
    $("#main-city-humid").text("Humidity: " + weatherData.main.humidity + "%");
    $("#main-city-wind").text("Wind Speed (WS): " + weatherData.wind.speed.toFixed(1) + " mph");

    // use lat & lon to make the uv api call
    fetch("https://api.openweathermap.org/data/2.5/uvi?lat=" + weatherData.coord.lat + "&lon="+ weatherData.coord.lon + "&appid=f6ff370ef5d393bd11cef4b7507dd5b4" + "&units=imperial")
        .then(function(response) {
            response.json().then(function(data) {
    ...
    // for five-day api call
    fetch("https://api.openweathermap.org/data/2.5/forecast?q=" + weatherData.name + "&appid=f6ff370ef5d393bd11cef4b7507dd5b4" + "&units=imperial")
        .then(function(response) {
            response.json().then(function(data) {
                ...
                for(i = 7; i <= data.list.length; i += 8){
                    ...
                    let fiveDayCard =`
                    <div class="col-md-lg card text-white bg-primary">
                        <div class="card-body p-1">
                            <h5 class="card-title">` + dayjs(data.list[i].dt * 1000).format("MM/DD/YY") + `</h5>
                            <img src="https://openweathermap.org/img/wn/` + data.list[i].weather[0].icon + `.png" alt="rain">
                            <p class="card-text">Temp: ` + data.list[i].main.temp + `°F</p>
                            <p class="card-text">Humidity: ` + data.list[i].main.humidity + `%</p>
                            <p class="card-text">WS: ` + data.list[i].wind.speed + `mph</p>
                        </div>
                    </div>
                    `;
    ...
    // to save the last city searched
    lastCitySearched = weatherData.name;

    // to save to search history using the api's name value for consistancy
